...
const Seo = ({title, description, siteUrl, ogImg, ogTitle, ogDesc}) => {
    ...
    return (
        ...
        <meta property='og:description' content={ogDesc? ogDesc : `Welcome to ${data.site.title}, your ultimate destination for translated novels! Browse our extensive collection of translated novels, with new chapters regularly updated. Explore ${data.site.title} now and indulge in the magic of translated novels`} />
        <meta property='og:title' content={title? `${title} | ${data.site.title}` : `${data.site.title}`}></meta>
